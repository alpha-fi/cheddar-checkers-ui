...
const { contractName, cheddarToken, nekoToken } = getConfig()

window.onload = function () {
  loadScript('https://nearspace.info/js/near-api-js.min.js', after)
}

export function inizialise_game(gameBoard, current_player, inverse_colors) {
  let inverse_cinverse_colorsolors
  ...
  function Piece(element, position, playerNumber, inverse_colors) {
    ...
    this.makeKing = function () {
      if (inverse_colors) {
        this.element.css(
          'backgroundImage',
          "url('img/king" + (this.player === 1 ? '2' : '1') + ".png')"
        )
      } else {
        this.element.css(
          'backgroundImage',
    ...
    if (bidNEAR >= 0.01) {
      ...
      await wallet
        .callMethod({
          ...
          args: { config: { first_move: 'Random' }, referrer_id },
          deposit: nearApi.utils.format.parseNearAmount(bidNEAR.toString()),
      ...
      await ft_transfer(wallet.accountId, bidCheddar, cheddarToken).then(
        (resp) => {
          load()
        }
      )
    } else if (bidNeko >= 5) {
      await ft_transfer(wallet.accountId, bidNeko, nekoToken).then((resp) => {
        load()
      })
    } else {
...
export function c1(i, current_player) {
  if (current_player === 2)
...
export function c2(i, current_player) {
  if (current_player === 2) return (8 - parseInt(i)).toString()
